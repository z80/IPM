  ...
  void qtluaopen_qt(State &ls)
  {
    ...
    static class : public Function
    {
        Value::List meta_call(State &ls, const Value::List &args)
        {
            ...
            meta_call_check_args( args, 3, 4, Value::TUserData, Value::TString, Value::TNone, Value::TNone );
            ...
            // We need 4 arguments.
            // For Qt method: object, signal string, object, slot string
            // For Lua method: object, signal string, self object, Lua function.
            // self object is an object which is substituted to Lua function as it's 
            // first argument.
            if ( args.size() < 4 )
                throw String( "At least 4 arguments expected." );
            ...
            switch ( args[3].type() )
	        {
	            case Value::TFunction:
                {
	                ...
	                sigqow->_lua_connect( sigindex, args[2], args[3] );
	                return Value::List();
	            }
