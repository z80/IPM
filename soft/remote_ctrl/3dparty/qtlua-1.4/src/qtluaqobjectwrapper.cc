...
namespace QtLua {
  ...
  int QObjectWrapper::qt_metacall(QMetaObject::Call c, int id, void **qt_args)
  {
    ...
    // Place "self" argument.
    lua_args.push_back( i.value()._self );
  ...
  void QObjectWrapper::_lua_connect( int sigindex, const Value & self, const Value & value )
  {
    switch (value.type())
      {
	...
	if (QMetaObject::connect(_obj, sigindex, this, metaObject()->methodCount() + slot_id))
	  {
	    _lua_slots.insert( slot_id, LuaSlot( self, value, sigindex ) );
	    return;
	  }
