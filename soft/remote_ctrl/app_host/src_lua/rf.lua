sleepTime = 0.5                              --Частота проверки состояния джойстиков

dbetaMaxPerSecond = 10 * math.pi / 180       --Максимальные изменения параметров за секунду
dalpha1MaxPerSecond = 10 * math.pi / 180
dalpha2MaxPerSecond = 10 * math.pi / 180
dl1MaxPerSecond = 15
decMaxPerSecond = 10                        --Максимальное изменение декартовой координаты за секунду

dbetaMax = dbetaMaxPerSecond * sleepTime     --Максимальные изменения параметров за иттерацию
...
li = 100                            --Нижняя граница l1
lt = 200                            --Верхняя граница l1
l2 = 10                             --Длина плеча l2
...
function main()
    ...
    while true do
        ...
        local x_joy3, y_joy3 = insensitivity( joy( 3 ) )    --Снятие показаний с 3-го джойстика с учетом зоны нечувствительности
        local x_joy4, y_joy4 = insensitivity( joy( 4 ) )    --Снятие показаний с 4-го джойстика с учетом зоны нечувствительности
        ...
        if x_joy3 ~= 0 or y_joy3 ~= 0 or x_joy4 or y_joy4 ~= 0 then --Если есть отклонения 3-го и 4-го джойстиков
            power_dbeta, power_dalpha1, power_dalpha2, power_dl1, vbeta, valpha1, valpha2, vl1 = getPower(beta, alpha1, alpha2, l1, x_joy3, y_joy3, x_joy4, y_joy4)             --Получение значений мощностей двигателей и обобщенных скоростей
            beta, alpha1, alpha2, l1 = moveManipulator(beta, alpha1, alpha2, l1, power_dbeta, power_dalpha1, power_dalpha2, power_dl1)          --Перемещение конечной точки манипулятора при заданных мощностях
        end
        
        x0, y0, z0 = dec_from_gen(beta, alpha1, alpha2, l1) --Положение конечной точки в ДСК, связянной с роботом
...
function get_power_dl1(length)                       --Получение мощности в процентах для мотора шарнира l1
    return length / dl1Max * 100
end

function get_power_dbeta(angle)                      --Получение мощности в процентах для мотора шарнира beta
    return angle / dbetaMax * 100
end

function get_power_dalpha1(angle)                    --Получение мощности в процентах для мотора шарнира alpha1
    return angle / dalpha1Max * 100
end

function get_power_dalpha2(angle)                    --Получение мощности в процентах для мотора шарнира alpha2
    return angle / dalpha2Max * 100
end

function change_l1(length, power)                       --Изменить параметр шарнира l1
    return length + dl1Max * power / 100
end

function change_beta(angle, power)                      --Изменить параметр шарнира beta
    return angle + dbetaMax * power / 100
end

function change_alpha1(angle, power)                    --Изменить параметр шарнира alpha1
    return angle + dalpha1Max * power / 100
end

function change_alpha2(angle, power)                    --Изменить параметр шарнира alpha2
    return angle + dalpha2Max * power / 100
end

function dec_from_gen(b, a1, a2, length1)               --Получение декартовых координат конечной точки манипулятора через обобщенные
...
function parallel_gen_from_dec(x, y, z)                 --Получение обобщенных координат через декартовые в паралелльной задаче
...
function orthogonal_gen_from_dec(x, y, z)               --Получение обобщенных координат через декартовые в перпендикулярной задаче
    local b
...
function orthogonal_gen_speed(dx, dy, dz, beta, alpha1, alpha2, l1) --Получение обобщенных скоростей
...
function check_if_b_boundary(angle)                 --Проверка на концевую точку и возвращение допустимого изменения координаты beta
...
function check_if_a1_boundary(angle)                --Проверка на концевую точку и возвращение допустимого изменения координаты alpha
...
function check_if_l1_boundary(length)               --Проверка на концевую точку и возвращение допустимого изменения координаты l1
...
function lowest_percent(p1, p2, p3)                 --Нахождение наименьшего числа
    local p = p1
...
function getPower(beta, alpha1, alpha2, l1, x_joy3, y_joy3, x_joy4, y_joy4)  --Получение значений мощностей двигателей и обобщенных скоростей
    local x1, y1, z1 = dec_from_gen(beta, alpha1, alpha2, l1)       --Узнаем текущие декартовые координаты
    ...
    if x_joy3 ~= 0 or y_joy3 ~= 0 or y_joy4 ~= 0 then
        ...
        dx = (x_joy3 / 100) * decMax                --Получаем предпологаемые перемещения за период опроса
        ...
        local beta_temp, alpha1_temp, alpha2_temp, l1_temp = orthogonal_gen_from_dec(x2, y2, z2)    --Узнаем предпологаемые обобщенные координаты после смещения на dx, dy, dz
...
function moveManipulator(beta, alpha1, alpha2, l1, power_dbeta, power_dalpha1, power_dalpha2, power_dl1)    --Перемещение конечной точки манипулятора при заданных мощностях
...
function insensitivity(x, y)                    --Проверка на зоны нечувствительности
