...
namespace Galois{
    ...
    Polynomial *Polynomial::solve(Polynomial *lu)
    {
        ...
        for(j=0;j<lu->cols;j++){
            for(i=j+1;i<lu->cols;i++){
                Nomial n = *ret->get(i)-*ret->get(j)**lu->get(i,j);
                ret->set(i,&n);
            }
        }
        for(j=lu->cols-1;j>=0;j--){
            for(int i=j+1;i<lu->cols;i++){
                Nomial n = *ret->get(j)-*lu->get(j,i)**ret->get(i);
                ret->set(j,&n);
            }
            Nomial n = *ret->get(j)/(*lu->get(j,j));
            ret->set(j,&n);
        }
        return(ret);
    }
    ...
    int BCH::decode(int syndrome_base)
    {
        ...
        if(err){
            ...
            for(j=0,c=0;j<this->rows;j++){
                ...
                for(i=1;i<errors;i++){
                    Nomial n = *sigma+*err->get(i)**this->_gf->exp2nomial(j*i);
                    sigma=&n;
                }
                Nomial n =*sigma+*this->_gf->exp2nomial(j*i);
                sigma=&n;
    ...
    Galois::Nomial *BCH::_error_syndrome(int d)
    {
        ...
        for(int i=0;i<this->rows;i++){
            Nomial n = *x+*this->get(i)**this->_gf->exp2nomial(i*d);
            x=&n;
        }
